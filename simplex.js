...
function procuraPivotColuna(tab) {
    let j, pivot_col = 1;
...
function procuraPivotLinha(tab, pivot_col) {
...
function variavelnaoBasica(tab) {
    let i, j;
    let rest = 0;
...
function calculaLinhaZ(tab, final) {
    var newZ = [];
    if (!final) {
        for (i = 0; i < tab.n; i++) {
            newZ.push(0);
...
function simplex(tab, iteracoes) {
    ...
    try {
        ...
        variavelnaoBasica(tab);
        passoapasso.push(copia(tab));
        do {
            if(loop > 20) return false;
            if (loop >= iteracoes) break;

            if (!tab.max) {
                if (!loop && tab.hasExtraVariable) calculaLinhaZ(tab);
                pivot_col = procuraPivotColuna(tab);
                if (pivot_col < 0) {
                    tab.max = true;
                    calculaLinhaZ(tab, true);
                    break;
                }
                pivot_row = procuraPivotLinha(tab, pivot_col);
                if (pivot_row < 0) {
                    tab.max = true;
                    if (tab.hasExtraVariable) calculaLinhaZ(tab, true);
                    break;
                }
                chamadaPivot(tab, pivot_row, pivot_col);
            }
            else {
                pivot_col = procuraPivotColuna(tab);
                if (pivot_col < 0) {
